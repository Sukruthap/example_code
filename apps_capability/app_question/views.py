...
#-------------------------------Adding question,choice,answer api function---------------------------------------------------------
@csrf_exempt
@api_view(['POST'])
def add_qac(request):
...
# ---------------------------------edit getting question,ans,choice---------------------------------------------------------------------
@csrf_exempt
@api_view(['GET'])
...
def update_question_choiceans(request,quesid):
    if request.method == 'PUT':
        ...
        try:
            ...
            quesinfo.save()
            if data['choices']:
                for i in range(0,len(ques_choices)-1):
                    if i == 0:
                        chid = ques_choices[i].id
                        try:
                            choiceobj = question_choices.objects.get(id=chid)
                            choiceobj.name = data['choices'][0]['name']
                            choiceobj.save()
                            pass
                            # print choiceobj
                        except question_choices.DoesNotExist:
                        ...
                        try:
                            choiceobj = question_choices.objects.get(id=chid)
                            choiceobj.name = data['choices'][1]['name']
                            choiceobj.save()
                            pass
                            # print choiceobj
                        except question_choices.DoesNotExist:
                        ...
                        try:
                            choiceobj = question_choices.objects.get(id=chid)
                            choiceobj.name = data['choices'][2]['name']
                            choiceobj.save()
                            pass
                            # print choiceobj
                        except question_choices.DoesNotExist:
                        ...
                        try:
                            choiceobj = question_choices.objects.get(id=chid)
                            # print choiceobj
                            choiceobj.name = data['choices'][3]['name']
                            choiceobj.save()
                            pass
                            # print choiceobj
                        except question_choices.DoesNotExist:
                            return Response(status=status.HTTP_404_NOT_FOUND)
                ques_choicestwo = question_choices.objects.filter(question_id=quesinfo)
                # print ques_choicestwo
                # print ques_ans, len(ques_ans)
                for j in range(len(ques_ans)):
                    if j == 0:

                        ansid = ques_ans[j].id
                        try:
                            ansobj = question_answer.objects.get(id=ansid)
                            choiceobj = ques_choicestwo.get(name=data['answer'][0]['name'])
...
# --------------------------get the count of perticular topic and details of question------------------------------------------------------------------
@csrf_exempt
@api_view(['GET'])
    ...
    if request.method == 'GET':
        try:
            ...
            for topobj in ques_topdetail:
                ...
                try:
                    topnewdict['image'] = topobj.image.url
                except:
